...
function toYaml(obj, alreadyIndented = true, indent = 0) {
	...
	return (alreadyIndented ? "" : `\n${indStr}`) +  Object.keys(obj).map((k, idx) => `${k}: ${toYaml(obj[k], idx == 0 && !alreadyIndented, indent + 2)}`).join(`\n${indStr}`)
}
...
window.view = new EditorView(document.querySelector("#editor"), {
	state: EditorState.create({
		doc: Node.fromJSON(gateSchema, {"type":"doc","content":[{"type":"title","content":[{"type":"text","text":"Jetzt Artikel freischalten:"}]},{"type":"accordion","content":[{"type":"title","content":[{"type":"text","text":"Digital-Zugang"}]},{"type":"list","content":[{"type":"listItem","content":[{"type":"paragraph","content":[{"type":"text","text":"Alle Artikel auf ZEIT ONLINE frei"}]}]},{"type":"listItem","content":[{"type":"paragraph","content":[{"type":"text","text":"Über 3.000 Rezepte im Wochenmarkt"}]}]},{"type":"listItem","content":[{"type":"paragraph","content":[{"type":"text","text":"Zusätzlich jeden Mittwoch die Wochenausgabe als E-Paper"}]}]},{"type":"listItem","content":[{"type":"paragraph","content":[{"type":"text","text":"4 Wochen für 1,00 € testen"}]}]}]},{"type":"button","content":[{"type":"text","text":"Jetzt für 1,00 € testen"}]}]}]}),
		plugins: [
			inputRules({ rules: [wrappingInputRule(/^\s*([-*])\s$/, gateSchema.nodes.list)] }),
			keymap({
				Enter(state, dispatch) {
					...
					if (node.type == gateSchema.nodes.button) {
						if (dispatch) {
							let tr = state.tr.insert($from.after(1), gateSchema.nodes.accordion.createAndFill())
